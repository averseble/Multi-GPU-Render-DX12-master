...
void HybridGrassSimApp_ImGuiSrvAllocFn(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE* out_cpu_handle,
                                       D3D12_GPU_DESCRIPTOR_HANDLE* out_gpu_handle)
{
    auto* app = static_cast<HybridGrassSimApp*>(info->UserData);
...
void HybridGrassSimApp_ImGuiSrvFreeFn(ImGui_ImplDX12_InitInfo* info, D3D12_CPU_DESCRIPTOR_HANDLE,
...
HybridGrassSimApp::HybridGrassSimApp(const HINSTANCE hInstance): D3DApp(hInstance)
{
    mSceneBounds.Center = Vector3(0.0f, 0.0f, 0.0f);
    mSceneBounds.Radius = 200;
}

HybridGrassSimApp::~HybridGrassSimApp()
{
    Flush();
    ShutdownImGui();
}

void HybridGrassSimApp::EnablePerformanceTestMode(int warmupSeconds, int sampleSeconds)
{
...
void HybridGrassSimApp::EnablePerformanceSweepMode(int warmupSeconds, int sampleSeconds)
{
    performanceTestMode = true;
...
void HybridGrassSimApp::Update(const GameTimer& gt)
{
    if (imguiInitialized)
    {
...
void HybridGrassSimApp::PopulateShadowMapCommands(std::shared_ptr<GCommandList> cmdList)
{
...
void HybridGrassSimApp::PopulateNormalMapCommands(const std::shared_ptr<GCommandList>& cmdList)
{
    //Draw Normals
    {
...
void HybridGrassSimApp::PopulateAmbientMapCommands(const std::shared_ptr<GCommandList>& cmdList)
{
    //Draw Ambient
    {
        cmdList->SetDescriptorsHeap(&srvTexturesMemory);
...
void HybridGrassSimApp::PopulateForwardPathCommands(const std::shared_ptr<GCommandList>& cmdList)
{
...
void HybridGrassSimApp::PopulateDrawCommands(std::shared_ptr<GCommandList> cmdList,
                                             RenderMode type)
{
...
void HybridGrassSimApp::PopulateInitRenderTarget(const std::shared_ptr<GCommandList>& cmdList, GTexture& renderTarget,
...
void HybridGrassSimApp::PopulateDrawFullQuadTexture(const std::shared_ptr<GCommandList>& cmdList,
...
void HybridGrassSimApp::Draw(const GameTimer& gt)
{
...
bool HybridGrassSimApp::Initialize()
{
    ...
    try
    {
        ...
        if (!InitMainWindow())
        {
            step("InitMainWindow failed");
            return false;
        }

        step("Flush after window");
        Flush();

        step("LoadStudyTexture");
        LoadStudyTexture();
        Flush();
        step("LoadModels");
        LoadModels();
        Flush();
        step("CreateMaterials");
        CreateMaterials();
        Flush();
        step("MipMasGenerate");
        MipMasGenerate();
        ...
        step("InitRenderPaths");
        InitRenderPaths();
        Flush();
        step("InitSRVMemoryAndMaterials");
        InitSRVMemoryAndMaterials();
        Flush();
        step("InitRootSignature");
        InitRootSignature();
        Flush();
        step("InitPipeLineResource");
    ...
    {
        step("DxException in Initialize");
        MessageBoxW(nullptr, e.ToString().c_str(), L"MGPU-GrassSim DxException", MB_OK | MB_ICONERROR);
        throw;
    }
    catch (const std::exception& e)
    {
        step(e.what());
        MessageBoxA(nullptr, e.what(), "MGPU-GrassSim init failed", MB_OK | MB_ICONERROR);
        return false;
    }
...
void HybridGrassSimApp::InitDevices()
{
...
bool HybridGrassSimApp::TryCreateCrossAdapterFences()
{
...
void HybridGrassSimApp::InitFrameResource()
{
...
void HybridGrassSimApp::InitRootSignature()
{
...
void HybridGrassSimApp::InitPipeLineResource()
{
    defaultInputLayout =
    {
        {
...
void HybridGrassSimApp::CreateMaterials()
{
...
void HybridGrassSimApp::InitSRVMemoryAndMaterials()
{
    srvTexturesMemory =
...
void HybridGrassSimApp::InitRenderPaths()
{
...
void HybridGrassSimApp::LoadStudyTexture()
{
...
void HybridGrassSimApp::LoadModels()
{
    auto queue = primeDevice->GetCommandQueue(GQueueType::Compute);
    const auto cmdList = queue->GetCommandList();
...
void HybridGrassSimApp::MipMasGenerate()
{
    try
    {
...
void HybridGrassSimApp::CreateGO()
{
    logQueue.Push(std::wstring(L"\nStart Create GO"));
...
void HybridGrassSimApp::CalculateFrameStats()
{
...
void HybridGrassSimApp::WritePerformanceTestResults()
{
...
void HybridGrassSimApp::WritePerformanceSweepResults()
{
...
void HybridGrassSimApp::LogWriting()
{
...
void HybridGrassSimApp::UpdateMaterials()
{
    {
...
void HybridGrassSimApp::UpdateShadowTransform(const GameTimer& gt)
{
...
void HybridGrassSimApp::UpdateShadowPassCB(const GameTimer& gt)
{
...
void HybridGrassSimApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void HybridGrassSimApp::UpdateSsaoCB(const GameTimer& gt)
{
...
bool HybridGrassSimApp::InitMainWindow()
{
...
void HybridGrassSimApp::OnResize()
{
    D3DApp::OnResize();
...
void HybridGrassSimApp::InitImGui()
{
    ...
    init_info.SrvDescriptorAllocFn = HybridGrassSimApp_ImGuiSrvAllocFn;
    init_info.SrvDescriptorFreeFn = HybridGrassSimApp_ImGuiSrvFreeFn;
...
void HybridGrassSimApp::ShutdownImGui()
{
    if (!imguiInitialized)
        return;
...
void HybridGrassSimApp::GetGrassWindFieldExtents(float& outCenterX, float& outCenterZ, float& outHalfExtent) const
{
    outCenterX = 0.0f;
    outCenterZ = 0.0f;
...
bool HybridGrassSimApp::TryPickGrassGroundFromMouse(const int clientX, const int clientY, Vector3& outHitWorld) const
{
...
void HybridGrassSimApp::EnsureWindGradientPreviewTexture()
{
...
void HybridGrassSimApp::InitWindGradientPreviewTexture()
{
...
void HybridGrassSimApp::ReleaseWindGradientPreviewTexture()
{
    windGradientPreviewTexture.Reset();
...
void HybridGrassSimApp::EnsureWindFluidReadbackMatchesVelocity(ID3D12Resource* velocityTex)
{
...
void HybridGrassSimApp::AppendWindFluidPreviewReadbackIfDue(const std::shared_ptr<GCommandList>& cmdList)
{
...
bool HybridGrassSimApp::TryRebuildWindGradientPreviewFromSecondGpu(UINT8* mapped)
{
...
void HybridGrassSimApp::RefreshWindGradientPreviewTexture(const std::shared_ptr<GCommandList>& cmdList)
{
...
void HybridGrassSimApp::DrawImGui(const std::shared_ptr<GCommandList>& cmdList)
{
...
LRESULT HybridGrassSimApp::MsgProc(const HWND hwnd, const UINT msg, const WPARAM wParam, const LPARAM lParam)
{
